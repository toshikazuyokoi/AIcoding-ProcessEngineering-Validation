...
/**
 * Backend .dockerignore Test Script
 * TSK-008-ENV-backend-dockerignore テスト実行
 * 
 * このスクリプトはbackend/.dockerignoreの内容と構成を検証します
 */

const fs = require('fs');
const path = require('path');

class BackendDockerignoreTest {
    constructor() {
        this.testResults = [];
        this.dockerignorePath = path.join(process.cwd(), 'backend/.dockerignore');
    }
     ...
     */
    async runTests() {
        console.log('🧪 Backend .dockerignore テスト開始');
        console.log('==========================================');
    ...
    async testDevelopmentExclusions() {
        ...
        try {
            ...
            const checks = [
                ...
                { condition: content.includes('*.test.ts'), name: 'TypeScriptテストファイル除外' },
                { condition: content.includes('*.spec.js'), name: 'JavaScriptスペックファイル除外' },
                { condition: content.includes('test-results/'), name: 'テスト結果除外' },
                { condition: content.includes('.vscode/'), name: 'VSCode設定除外' }
            ];
    ...
    async testInfrastructureExclusions() {
        ...
        try {
            ...
            const checks = [
                ...
                { condition: content.includes('.gitlab-ci.yml'), name: 'GitLab CIファイル除外' },
                { condition: content.includes('Jenkinsfile'), name: 'Jenkinsファイル除外' },
                { condition: content.includes('.circleci/'), name: 'CircleCIディレクトリ除外' }
            ];
...
if (require.main === module) {
    const test = new BackendDockerignoreTest();
    test.runTests().then(success => {
        process.exit(success ? 0 : 1);
    });
}

module.exports = BackendDockerignoreTest;
